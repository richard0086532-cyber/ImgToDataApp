...
# ==============================================================================
# PaddleOCR 环境变量（延迟到 _init_ocr 中设置）

# PaddleOCR
# paddleocr 延迟导入（避免启动时崩溃）
# from paddleocr import PaddleOCR  # 移到 _init_ocr 方法内部
...
class ChartExtractorAgent:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self._init_ocr()
        # 如果 OCR 初始化失败，标记为不可用但允许对象存在
        self._ocr_available = self.ocr_engine is not None

    def _init_ocr(self):
        """初始化RapidOCR引擎（轻量版，内存占用~100MB，适配Render免费版）"""
        try:
            from rapidocr_onnxruntime import RapidOCR
        except ImportError as e:
            print(f"[ERROR] RapidOCR 未安装: {e}")
            print("[INFO] 请执行: pip install rapidocr-onnxruntime")
            self.ocr_engine = None
            return
        ...
        try:
            ...
            print("[INFO] RapidOCR引擎初始化完成（轻量ONNX模型）")
    ...
    def _run_ocr(self, img: np.ndarray) -> List:
        """
        统一OCR调用入口（RapidOCR轻量版）。
        返回格式: [[bbox, (text, score)], ...]
        """
        if self.ocr_engine is None:
            print("[WARN] OCR 引擎未初始化，尝试重新初始化...")
            self._init_ocr()
            if self.ocr_engine is None:
                print("[ERROR] OCR 引擎初始化失败，返回空结果")
                return []
        ...
        for item in result:
            ...
            if len(item) >= 3:
                bbox = item[0]
                text = str(item[1])
                score = float(item[2]) if item[2] is not None else 1.0
            else:
                continue
    ...
    def process(self, image_input, output_dir: str = "./output", progress_callback=None) -> ExtractionResult:
        ...
        result = ExtractionResult(image_path=base_name)
        self.panel_results = []

        try:
            if not getattr(self, '_ocr_available', False):
                print("[ERROR] OCR 引擎不可用，无法处理图像")
                result.warnings.append("OCR 引擎未安装或未初始化，请检查依赖")
                result.is_valid = False
                return result

            print(f"\n{'='*60}")
            print(f"[START] 处理图像: {base_name}")
            print(f"{'='*60}")
...
def save_task(task_id, data):
    ...
    with _lock:
        _tasks[task_id] = data
        filepath = os.path.join(RESULTS_DIR, f"{task_id}_status.json")
        try:
            with open(filepath, 'w', encoding='utf-8') as f:
                json.dump(data, f, ensure_ascii=False, default=str)
        except Exception as e:
            print(f"[WARN] 保存任务状态失败: {e}")
...
def run_extraction_task(task_id, image_bytes):
    """后台线程执行图表提取"""
    ...
    try:
        ...
        with open(csv_path, 'w', encoding='utf-8') as f:
            f.write(web_result.get("csv_data", ""))
